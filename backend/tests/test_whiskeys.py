...
class TestListWhiskeys:
    ...
    def test_search_by_name(self, client, sample_whiskeys):
        ...
        assert len(items) == 1
        assert items[0]["name"] == "Buffalo Trace"
    ...
    def test_filter_by_category(self, client, sample_whiskeys):
        ...
        assert resp.status_code == 200
        items = resp.json()["items"]
        assert all("bourbon" in w["category"].lower() for w in items)
    ...
    def test_pagination(self, client, sample_whiskeys):
        ...
        items1 = resp.json()["items"]
        assert len(items1) == 2
        resp2 = client.get("/whiskeys/", params={"skip": 2, "limit": 2})
        items2 = resp2.json()["items"]
        assert len(items2) == 2
        # No overlap
        ids1 = {w["id"] for w in items1}
        ids2 = {w["id"] for w in items2}
