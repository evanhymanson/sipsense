...
def create_refresh_token(username: str, remember_me: bool = False) -> str:
    now = datetime.now(timezone.utc)
    expire_minutes = REFRESH_TOKEN_REMEMBER_MINUTES if remember_me else REFRESH_TOKEN_EXPIRE_MINUTES
    payload = {
        "sub": username,
...
def decode_refresh_token(token: str) -> tuple[str, bool] | None:
    """Returns (username, remember_me) from a valid refresh token, or None."""
    try:
        payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
        ...
        if not username:
            return None
        return username, payload.get("remember", False)
    except JWTError:
        return None
